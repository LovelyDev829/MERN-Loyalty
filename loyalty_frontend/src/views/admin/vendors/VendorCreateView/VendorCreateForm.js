  ...
  return (
    <Formik
      ...
      onSubmit={async (values, {
        ...
        try {
          const response = await axios.post(`${API_BASE_URL}/admin/vendor`, values);

          setStatus({ success: response.data.status });
          setSubmitting(false);
          enqueueSnackbar('Vendor created', {
            variant: 'success',
            action: <Button>See all</Button>
          });

          if (!isPopup) {
            // history.push('/app/management/vendors');
          } else {
            setModalIsOpenToFalse();
          }
        } catch (error) {
