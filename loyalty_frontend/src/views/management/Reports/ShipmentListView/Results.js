...
import { SET_SHIPMENTS } from 'src/actions/shipmentActions';
...
const Results = forwardRef(({
  ...
  const handleExportToCSVMainLogic = () => {
    const queryData = {
      shipmentIdArr: selectedShipments,
    };
    const data = [];
    axios.post(`${API_BASE_URL}/shipment/load`, queryData)
      .then((response) => {
        for (let i = 0; i < response.data.shipments.length; i++) {
          const shipmentOne = response.data.shipments[i];
          const dataOne = {};
          dataOne.customer = `${shipmentOne.customer[0].firstName} ${shipmentOne.customer[0].lastName}`;
          dataOne.invoiceDate = moment(shipmentOne.dueStart).format('MM/DD/YYYY');
          dataOne.dueDate = '';
          dataOne.terms = '';
          dataOne.memo = convertToPlain(shipmentOne.customerNote);
          dataOne.item = shipmentOne.type === '1' ? 'Shipment' : 'Service';
          dataOne.itemDescription = shipmentOne.title;
          dataOne.itemQuantity = '1';
          dataOne.itemRate = shipmentOne.price;
          dataOne.itemAmount = shipmentOne.price;
          dataOne.taxable = '';
          dataOne.taxRate = '';
          dataOne.serviceDate = '';
          data.push(dataOne);
  ...
  //   if (rtn_status) {
  //     enqueueSnackbar(type === '1' ? 'Shipments deleted' : 'Services deleted', {
  //       variant: 'success',
  ...
  return (
    <>
          ...
          <div className={classes.bulkOperations}>
            <div className={classes.bulkActions}>
              ...
              <Button
                ...
                onClick={() => handleExportToCSVMainLogic()}
              >
                Export Reports
              </Button>
              {/* <Button
      ...
      <CSVLink
        ...
        headers={headers}
        style={{ display: 'none' }}
        filename={`Shipment-Reports-(${moment(new Date()).format('YYYY-MM-DD')}).csv`}
        ref={csvDownloadLink}
        target="_blank"
      >
        Download me
