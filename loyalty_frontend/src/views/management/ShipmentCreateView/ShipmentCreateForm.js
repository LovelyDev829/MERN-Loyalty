...
import { getDefaultDateTime } from 'src/utils/helper';
...
function ShipmentCreateForm({
  ...
  const updateMyDataWithSelect = (rowIndex, columnId, value, setFieldValue) => {
    ...
    setFieldValue('packagesTableDataSP', editableTableDataWithSelect);
  };
  ...
  return (
    <>
      <Formik
        initialValues={{
          type,
          title: type === '1' ? `BOL#${genRanHex(4)}` : `SO#${genRanHex(4)}`,
          customerNote: '',
          internalNote: '',
          dueStart: getDefaultDateTime(),
          dueEnd: getDefaultDateTime(),
          originCompany: '',
          originFullAddr: '',
          originAptSuite: '',
          originCity: '',
          ...
          destPostalCode: '',
          isActive: true,
          packagesTableData: editableTableData,
          vendorsTableData: editableTableDataWithSelectVendor,
          status: '1', // New
          insured: 0,
          declared: 0,
          price: 0,
        ...
        onSubmit={async (values, {
          ...
          try {
            if (isPopup) {
              values.customer = customerFromParent.id;
            }
            ...
            const response = await axios.post(`${API_BASE_URL}/shipment`, values);

            setStatus({ success: response.data.status });
            setSubmitting(false);
            enqueueSnackbar(type === '1' ? 'Shipment created' : 'Service created', {
              variant: 'success',
              action: <Button>See all</Button>
            });
      ...
      >
        {({
          ...
          >
            ...
            {/* <input type="hidden" value={values.packagesTableData} name="packagesTableData" /> */}
            <Card>
              <CardContent>
                <Box mt={2} className={classes.buttonWraper}>
                  ...
                  >
                    {type === '1' ? 'Create Shipment' : 'Create Service'}
                  </Button>
                </Box>
                <Grid
                  container
                    ...
                    >
                        ...
                        >
                          ...
                          >
                            ...
                            {isPopup && (
                              <>
                                <Typography
                                  className={clsx(classes.ShipmentIDLabel, classes.marginTopSeven)}
                                  variant="body2"
                                  color="textSecondary"
                                >
                                  Customer:
                                ...
                                <Label
                                  className={clsx(classes.labelForValue, classes.marginTopSeven)}
                                  color="#000"
                                >
                                  {customerFromParent.firstName}
                                  {' '}
                                  {customerFromParent.lastName}
                                  {' ('}
                                  {customerFromParent.email}
                                  {')'}
                                </Label>
                              </>
                            )}
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Label
                              className={classes.labelForValue}
                              color="#000"
                            >
                              &nbsp;
                            </Label>
                          ...
                          >
                            <DateTimePicker
                              ...
                              className={classes.marginTopFive}
                            />
                          </Grid>
                          <Grid
                            item
...
ShipmentCreateForm.propTypes = {
  className: PropTypes.string,
  shipment: PropTypes.object,
  isPopup: PropTypes.bool,
  setModalIsOpenToFalse: PropTypes.func,
  customerFromParent: PropTypes.object,
